...
def get_login_data():  # Method to fetch the Login data from the Excel sheet
    login_data = []
    # Read the Excel file into a DataFrame
    df = pd.read_excel('login_data.xlsx', 'Login')  # Update the path to your Excel file if needed
...
def update_excel(username, password, test_result, tester_name):  # Method to update the status in the Excel sheet
    ...
    temp_file_path = 'login_data_temp.xlsx'
    ...
    try:
        ...
        with pd.ExcelWriter(temp_file_path, engine='openpyxl', mode='w') as writer:
            df.to_excel(writer, index=False, sheet_name='Login')
